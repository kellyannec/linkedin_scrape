...
f = open(parameters.file_name, 'w', newline = '', encoding='utf8')
writer = csv.writer(f)
...
for linkedin_url in linkedin_urls:
    ...
    if not experience:
            experience = sel.xpath('//*[@class = "t-14 t-black t-bold"]/.//span[2]').extract_first()
            experience = experience.replace("<span>", "") if experience else None
            experience = experience.replace("</span>", "") if experience else None
            experience = ' '.join(experience.split()) if experience else None
            company = sel.xpath('//*[@class = "t-16 t-black t-bold"]/.//span[2]').extract_first()
            company = company.replace("<span>", "") if company else None
            company = company.replace("</span>", "") if company else None
            company = ' '.join(company.split()) if company else None
